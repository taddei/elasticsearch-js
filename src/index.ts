...
import { ResurrectEvent } from './pool/ConnectionPool'
...
type callbackFn<T> = (err: Error | null, result: ApiResponse<T>) => void
...
interface ApiMethod<TParams, TBody = any> {
  ...
  // Callback API
  (callback: callbackFn<TBody>): TransportRequestReturn
  (params: TParams, callback: callbackFn<TBody>): TransportRequestReturn
  (params: TParams, options: TransportRequestOptions, callback: callbackFn<TBody>): TransportRequestReturn
}
...
type extendsCallback = (options: ClientExtendsCallbackOptions) => any
...
interface ClientOptions {
  node?: string | NodeOptions
  nodes?: string[] | NodeOptions[]
...
class Client extends EventEmitter {
  ...
  constructor (opts: ClientOptions = {}) {
    ...
    if (opts.node !== undefined || opts.nodes !== undefined) {
...
function getAuth (node: string | string[] | NodeOptions | NodeOptions[]): { username: string, password: string } | null {
  ...
  function getUsernameAndPassword (node: string | NodeOptions): { username: string, password: string } {
