...
class Connection {
  ...
  buildRequestObject (params) {
    ...
    // if AWS credentials configured, sign the payload
    if (
      process.env.ES_AWS_ACCESS_KEY && 
      process.env.ES_AWS_ACCESS_KEY_SECRET && 
      process.env.ES_AWS_REGION
    ) {
      request.service = "es"
      request.region = process.env.ES_AWS_REGION

      aws4.sign(request, {
        accessKeyId: process.env.ES_AWS_ACCESS_KEY,
        secretAccessKey: process.env.ES_AWS_ACCESS_KEY_SECRET,
      });
    }
